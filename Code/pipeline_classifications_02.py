...
def evaluate_metrics(
    ...
    return {
        ...
        # ROC-AUC считаем по вероятностям
        "roc_auc": roc_auc_score(y_true, y_proba),
    }


def tune_multiple_metrics(
    ...
    logger.info("Подбор гиперпараметров с оценкой по нескольким метрикам...")
    ...
    for C in param_grid["C"]:
        for l1_ratio in param_grid["l1_ratio"]:
            for class_weight in param_grid["class_weight"]:
                ...
                cv = StratifiedKFold(
                    n_splits=cv_cnt, shuffle=True, random_state=rnd_seed
                )
    ...
    best_res = max(results, key=lambda x: x["f1_mean"])

    logger.info(
        f"------Лучший результат (по f1-score):------\n"
        + f"C={best_res['C']}, l1_ratio={best_res['l1_ratio']}, class_weight={best_res['class_weight']}\n"
        + f"F1: {best_res['f1_mean']:.4f} (+/- std={best_res['f1_std']:.4f})\n"
        + f"Precision: {best_res['precision_mean']:.4f}\n"
        + f"Recall: {best_res['recall_mean']:.4f}\n"
        + f"ROC-AUC: {best_res['roc_auc_mean']:.4f}\n"
    )

    return {
        "best_params": {
            "C": best_res["C"],
            "l1_ratio": best_res["l1_ratio"],
            "class_weight": best_res["class_weight"],
        },
...
def main() -> None:
    ...
    logger.info(
        f"Разбиение завершено. Train: {len(X_train)}, Test: {len(X_test)}"
    )

    tuning_results = tune_multiple_metrics(
        X_train, y_train, cv_cnt=5, rnd_seed=rnd_seed, max_iter=1_000
    )
    ...
    best_pipeline = Pipeline(
        [
            ...
            (
                "logregr",
                LogisticRegression(
                    l1_ratio=tuning_results["best_params"]["l1_ratio"],
                    solver="liblinear",
                    C=tuning_results["best_params"]["C"],
                    class_weight=tuning_results["best_params"]["class_weight"],
                    random_state=rnd_seed,
                ),
            ),
        ]
    ...
    logger.info("Обучение лучшей модели на всех тренировочных данных...")
    best_pipeline.fit(X_train, y_train)

    # Анализ зануленных признаков (Извлекаем обученную модель)
    zeroed_features = np.sum(best_pipeline["logregr"].coef_ == 0)
    ...
    # так как возвращает 2-мерный массив вида [[p_1, p_0], ...]
